    ...
    <section className="mx-auto max-w-md space-y-4 p-6 stage-card-light">
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        <div className="space-y-1">
          ...
          <input id="full_name" name="full_name" required className="stage-input-light" />
        </div>
        <div className="space-y-1">
          <label className="text-sm text-slate-700" htmlFor="city">
            Город
          </label>
          <input id="city" name="city" required className="stage-input-light" />
        </div>
        <div className="space-y-1">
          <label className="text-sm text-slate-700" htmlFor="role">
            Я —
          </label>
          <select id="role" name="role" className="stage-input-light">
            <option value="client">
              Клиент
            </option>
            <option value="master">
        ...
        <div className="space-y-1">
          ...
          <input id="password" name="password" type="password" minLength={8} autoComplete="new-password" required className="stage-input-light" />
        </div>
        <div className="space-y-1">
          <label className="text-sm text-slate-700" htmlFor="password2">
            Повторите пароль
          </label>
          <input id="password2" name="password2" type="password" minLength={8} autoComplete="new-password" required className="stage-input-light" />
        </div>
        <label className="flex items-start gap-2 text-sm text-slate-700">
          <input name="agree" value="1" type="checkbox" className="mt-1" required />
          <span>
            Я согласен с <Link className="text-slate-900 underline decoration-slate-400/50 hover:decoration-slate-900" href="/terms">условиями</Link> и{" "}
            <Link className="text-slate-900 underline decoration-slate-400/50 hover:decoration-slate-900" href="/privacy">политикой конфиденциальности</Link>.
          </span>
        </label>
