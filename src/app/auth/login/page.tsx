...
function LoginForm() {
  ...
  return (
    <section className="mx-auto max-w-md space-y-4 p-6 stage-card">
      <h1 className="text-2xl font-semibold text-amber-200">Вход</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        ...
        <div className="space-y-1">
          ...
          <input id="password" name="password" type="password" autoComplete="current-password" required className="stage-input" />
        </div>
      ...
      <div className="flex items-center justify-between text-sm">
        <Link className="text-amber-200 underline" href="/auth/forgot">
          Забыли пароль?
        </Link>
        <span className="text-white/70">
          Нет аккаунта?{" "}
          <Link className="text-amber-200 underline" href="/auth/register">
            Регистрация
          </Link>
        </span>
      </div>
